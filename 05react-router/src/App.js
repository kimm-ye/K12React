...
react-router-dom
  ...
  렌더링하고, 그 상태를 관리하기 위해 내부적으로 state나 props를 관리할 수 있게
  해주는 도구가 react-router-dom이다.
*/
import './App.css';
...
Link
  : a 태그를 통한 링크는 클릭할때마다 화면의 새로고침되어 갱신된다.
  React는 화면의 깜빡임이 없어야하므로, 링크를 눌렀을때 reload되지 않도록 처리해준다.

라우터는 함수형이나 클래스형이나 사용가능하다.
*/
...
//Topics의 메뉴링크를 구성하기 위한 JSON배열 생성
let contents =[
...
/*
useParams() : 파라미터로 전송된 값을 받아오기 위한 리엑트 훅으로
    ...
    <BrowserRouter>
      <div className="App">
        ...
        exact
          ...
          만약 포함하지 않으면 Topics를 눌렀을때 Home이 보이는 문제가 발생한다.

        if나 장고의 url정의한것과 동일한 ...
        */}
        <Route exact path="/"><Home></Home></Route>
        <Route path="/Topics"><Topics></Topics></Route>
        <Route path="/Contact"><Contact></Contact></Route>

        {/* switch는 딱 맞는 하나만 매핑하고 그 이외엔 보지 않는다. 
        그래서 Home에 exact가 없으면 다 / 슬러쉬가 있으므로 클릭해도 Home에서 화면이 움직이지 않는다.*/}
        <h3>Switch적용</h3>
        {/* 
